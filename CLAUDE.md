...
├── e2e/                                    # Playwright specs + global-setup + seed-data (emulator e2e)
├── scripts/
│   └── seed-emulator.ts                    # Emulator seed (coach + trainees + meal plan + exercises)
├── docs/
│   └── testing.md                          # How to run the e2e suite (emulators + seeded users)
├── playwright.config.ts                    # Per-role projects + storageState + webServer
├── messages/
...
### Coach view of full onboarding data (Issue #6)
Everything the trainee entered at onboarding is stored in `users/{uid}.onboarding`
and already returned by `getTraineeDetails` — Issue #6 simply **surfaces all of it
to the coach** (display-only; no change to how it's stored). The stored shape is
typed as **`OnboardingData`** (`types/index.ts`, all fields optional) and referenced
by `QwaamUser.onboarding?`. The **same** `RegistrationCard` from Issue #5 is expanded
(not duplicated) to render labelled sections — **Personal / Health / Goals & Training /
Body & measurements** — plus the folded-in trained-before answer + guide photos, and
InBody/body-photo thumbnails (reusing the lightbox). It reads strictly from the stored
object and renders read-only; enum/boolean values map to localized labels
(`single/married`, `fatBurn/gainMuscle/gainWeight`, yes/no). Missing/optional fields
are omitted (per-section `has*` predicates + empty-`Row` collapse), never shown as
`undefined`; `injuryDetails`/`chronicDiseases`/measurements render only when their
gating flag/values are present. Labels live in the `coach` i18n namespace (ar + en).
No new fetch, no storage/action change.
...
|------|--------|---------------|
| 2026-07-08 | **Issue #7** — simplified the "Add exercise to library" form to catalog-only fields (nameAr, nameEn, targetMuscle, equipment, optional videoUrl). Removed the four training-default inputs (`defaultSets`/`defaultReps`/`defaultWeightLevel`/`defaultRest`) from `ExerciseForm`; made them **optional** on the `Exercise` type (kept, not removed — existing docs grandfathered); dropped the server-side `defaultSets`/`defaultReps` requirement in `addExercise`. Workout builder pre-fill + placeholders fall back to standard constants (`sets=3`, `reps=10-12`, `weight=medium`, `rest=60`) when an exercise has no defaults. `ExerciseBrowser` view chips + client `sets×reps` badge guarded against missing values (never render `undefined`). Client display uses the per-workout **override** values (not library defaults), so removing library defaults blanks nothing. `@smoke`: add-exercise form shows only the four kept fields. | `types/index.ts`, `LibraryContent.tsx`, `library-actions.ts`, `ExerciseBrowser.tsx`, `client/page.tsx`, `e2e/coach.spec.ts` |
| 2026-07-08 | **Coach UX** — `RegistrationCard` (trainee "بيانات التسجيل") is now **collapsible, collapsed by default** (accessible header button with `aria-expanded` + rotating chevron; title + icon always visible) so it doesn't dominate the trainee detail page. Issue #6 `@smoke` expands the card before asserting. | `RegistrationCard.tsx`, `e2e/coach.spec.ts` |
...
| 2026-07-08 | **Issue #6** — surface the FULL onboarding data to the coach (display-only). New `OnboardingData` type + `QwaamUser.onboarding?` (drops `as any`). Expanded existing `RegistrationCard` into labelled sections (Personal / Health / Goals & Training / Body & measurements) + folded-in trained-before + guide photos + InBody/body-photo thumbnails/lightbox. Read-only, enum/boolean → localized labels, graceful omit of empty fields/sections. No new fetch (reuses `getTraineeDetails`), no storage/action change. New `coach` i18n labels (ar+en). Seed trainee A given a representative `onboarding` object; new coach `@smoke` asserts the section renders the trainee's values. | `types/index.ts`, `RegistrationCard.tsx`, `admin/client/[id]/page.tsx`, `ar.json`, `en.json`, `scripts/seed-emulator.ts`, `e2e/coach.spec.ts` |
